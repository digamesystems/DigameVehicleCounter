...
void DEBUG_LOG(String message)
{
  if (config.logDebugEvents != "checked")
    return;
...
bool inTestMode = false; // When we are using the web UI for real-time testing, don't send messages to the server.
...
//*******************************************************************************
{
  static uint32_t availableHeap = 0;

  String strOutput = "\n";
  strOutput += "*** " + title + "\n";
  availableHeap = ESP.getFreeHeap();
  strOutput += " Heap:   " + String(availableHeap) + "\n";
  DEBUG_PRINT(strOutput);
  DEBUG_LOG(strOutput);
}

//*******************************************************************************
// Show the reason for the last reset. - Write to serial port and log file.
void show_reset_reason(int reason)
//*******************************************************************************
{
  String resetReason = "";
  DEBUG_PRINTLN();
  DEBUG_PRINTLN("----");
  DEBUG_PRINTLN("Reset reason: ");
  switch (reason)
  {
  ...
  case ESP_RST_SW:
    ...
    break;
  case ESP_RST_PANIC:
    resetReason = "Software reset due to exception/panic";
    break;
...
//****************************************************************************************
{
 ...
 // Configure the watchdog timer to reset the ESP32 if it hangs.
  ...
  esp_task_wdt_reset(); // Kick the watchdog timer in setup().
  ...
  show_reset_reason(reset_reason);
  esp_task_wdt_reset(); // Kick the watchdog timer in setup().
...
//****************************************************************************************
{
  ...
  if ((millis() - loopTimer) < 20)
  { // Adjust to c.a. 50 Hz loop rate.
...
void configureNetworking(String &statusMsg)
{
  ...
  else  // We are not in AP mode. Configure the network for normal operation.
  {
...
#if USE_WIFI
  if (wifiConnected)
  { // Attempt to synch ESP32 clock with NTP Server...
...
String buildLoRaJSONHeader(String eventType, double count, String lane = "1")
{
  ...
  if ((eventType == "b") || (eventType == "hb"))
  { // In the boot/heartbeat messages, send the current settings.
    loraHeader = loraHeader +
                 "\",\"s\":{" +
...
String buildWiFiJSONHeader(String eventType, double count, String lane = "1")
{
  ...
  if ((eventType == "Boot") || (eventType == "Heartbeat"))
  { // In the boot/heartbeat messages, send the current settings.
    jsonHeader = jsonHeader +
                 "\",\"settings\":{" +
                 "\"ui\":\"" + config.lidarUpdateInterval + "\"" +
...
{ // Issue a heartbeat message, if needed.
  //**************************************************************************************
...
{ // Test if vehicle event has occured. Route message if needed.
  //****************************************************************************************
  ...
  ****************** END STRESS TEST ********************************/
  ...
  if (vehicleMessageNeeded > 0)
  {
    ...
    if (lidarBuffer.size() == lidarSamples)
    { // Fill up the buffer before processing so
